...
### ① コンテンツ生成、質問応答
ユーザーの入力**（プロンプト）**に基づき、文章やコード、画像などのコンテンツを生成する。推論や Web 検索を活用する AI は複雑な問題にも対応できる。
...
### ③ 警告やエラーメッセージを含める
- 発生した警告やエラーメッセージを、コードブロック（言語指定なし）に書くことで、AI が内容を正しく解釈しやすくなる
- エラーメッセージはそのまま完全にコピーし、**一部だけを抜粋しないようにする**
...
- 利用するAIの種類（無料/有料）によって評価が有利・不利になることはない
...
- ページのソースを Markdown 形式で GitHub にて公開しています（ライセンス CC0）
- 自由に改変して活用してください